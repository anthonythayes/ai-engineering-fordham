...
def load_wands_products(data_dir: str = "../data") -> pd.DataFrame:
    ...
    products = products.rename(
        columns={"category hierarchy": "category_hierarchy"}
    )
    return products
...
def evaluate_search(
    ...
    for _, row in queries_df.iterrows():
        ...
        query_labels = labels_df[labels_df["query_id"] == query_id]
        relevant_ids = set(
            query_labels[query_labels["grade"] > 0]["product_id"]
        )
...
def batch_cosine_similarity(
    query_emb: np.ndarray, doc_embs: np.ndarray
) -> np.ndarray:
    """
